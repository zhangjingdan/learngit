...
BEGIN_MESSAGE_MAP(CFiveChessDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDOK, &CFiveChessDlg::OnBnClickedOk)
	ON_WM_SIZE()
END_MESSAGE_MAP()
...
void CFiveChessDlg::OnPaint()
{
	...
	else
	{
		CDialogEx::OnPaint();
		CDC* pDC = GetDC();
	CRect rect(20,470,470,20);
	my_Board.InitBoard(rect,14,14,pDC);
	my_Board.DrawBoard(pDC);
	}
}
...
void CFiveChessDlg::OnBnClickedOk()
{
	// TODO: 在此添加控件通知处理程序代码
	CDialogEx::OnOK();
}
...
void CFiveChessDlg::OnSize(UINT nType, int cx, int cy)
{
